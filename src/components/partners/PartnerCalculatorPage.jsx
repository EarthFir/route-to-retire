...
// is Route to Retire's own pitch to the adviser, not part of the fictional
// Harbour & Vale branding, so it stays on the site's own palette, styled
// dark like the calculator's own "Your Projection" results card. The
// standard rate is shown struck through against the early-adopter offer so
// it's clear this is an introductory price, not the only one that'll ever
// exist.
...
function BookDemoButton({ className, style }) {
  // Falls back to the on-page enquiry form until a real scheduling link
  // (e.g. Calendly) is set in businessInfo.js, so the button is never dead.
  ...
  return (
    <a href="#enquiry-form" className={className} style={style}>
      Get in touch
    </a>
  );
}
